...
public class OsmandApplication extends Application implements ClientContext {
	...
	public void onCreate() {
		...
		if (Version.getAppName(this).equals("OsmAnd~")) {
			if (android.os.Build.VERSION.SDK_INT >= 9) {
				try {
					Class.forName("net.osmand.plus.base.EnableStrictMode")
							.newInstance();
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		externalServiceAPI = new net.osmand.plus.api.ExternalServiceAPIImpl(
				this);
		...
		if (!osmandSettings.FOLLOW_THE_ROUTE.get()) {
			osmandSettings.APPLICATION_MODE
					.set(osmandSettings.DEFAULT_APPLICATION_MODE.get());
		}
	...
	public RoutingConfiguration.Builder getDefaultRoutingConfig() {
		if (defaultRoutingConfig == null) {
			...
			if (routingXml.exists() && routingXml.canRead()) {
				try {
					defaultRoutingConfig = RoutingConfiguration
	...
	public void checkApplicationIsBeingInitialized(Activity activity,
		...
		synchronized (OsmandApplication.this) {
			if (startDialog != null) {
				 ...
				 * progressDialog
				 * .setMessage(getString(R.string.reading_indexes));
	...
	public void showDialogInitializingCommandPlayer(final Activity uiContext,
		...
		if (voiceProvider == null
			...
			if (player == null
					|| !Algorithms.objectEquals(voiceProvider,
		...
		try {
			if (!Version.isBlackberry(this)) {
				if (osmandSettings.NATIVE_RENDERING_FAILED.get()) {
					...
					startDialog.startTask(
							getString(R.string.init_native_library), -1);
					RenderingRulesStorage storage = rendererRegistry
							.getCurrentSelectedRenderer();
					// boolean initialized =
					// NativeOsmandLibrary.getLibrary(storage, this) != null;
			...
			synchronized (OsmandApplication.this) {
				...
				if (toDismiss != null) {
					uiHandler.post(new Runnable() {
						...
						public void run() {
							if (toDismiss != null) {
								// TODO handling this dialog is bad, we need a
								// better standard way
								toDismiss.dismiss();
	...
	private class DefaultExceptionHandler implements UncaughtExceptionHandler {
		...
		public DefaultExceptionHandler() {
			...
			intent = PendingIntent.getActivity(OsmandApplication.this
					...
					new Intent(OsmandApplication.this.getBaseContext(),
							OsmandIntents.getMainMenuActivity()), 0);
		}
		...
		public void uncaughtException(final Thread thread, final Throwable ex) {
			...
			try {
				...
				msg.append(
						"Version  " + Version.getFullVersion(OsmandApplication.this) + "\n"). //$NON-NLS-1$ 
