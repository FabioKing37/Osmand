...
public class SettingsGeneralActivity extends SettingsBaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!Version.isBlackberry(getMyApplication())) {
			CheckBoxPreference nativeCheckbox = createCheckBoxPreference(
					settings.SAFE_MODE, R.string.safe_mode,
					R.string.safe_mode_description);
			// disable the checkbox if the library cannot be used
			if ((NativeOsmandLibrary.isLoaded() && !NativeOsmandLibrary
					.isSupported()) || settings.NATIVE_RENDERING_FAILED.get()) {
			...
			applicationDir
					.setOnPreferenceClickListener(new OnPreferenceClickListener() {
							...
							editalert.setPositiveButton(
									...
									new DialogInterface.OnClickListener() {
										public void onClick(
												...
												int whichButton) {
											warnAboutChangingStorage(input
													.getText().toString());
										}
									});
							editalert.show();
						...
						public boolean onPreferenceClick(Preference preference) {
							SuggestExternalDirectoryDialog.showDialog(
									SettingsGeneralActivity.this,
									new OnClickListener() {
										@Override
										public void onClick(
										...
										public void run() {
											Toast.makeText(
													SettingsGeneralActivity.this,
													getString(R.string.application_dir_change_warning),
		...
		// R.string.use_kalman_filter_compass,
		// R.string.use_kalman_filter_compass_descr));

		registerBooleanPreference(settings.USE_ENGLISH_NAMES, screen);

		// List preferences

		registerListPreference(settings.ROTATE_MAP, screen, new String[] {
		...
		registerListPreference(settings.OSMAND_THEME, screen, new String[] {
				"Dark", "Light" }, new Integer[] {
		...
		DayNightMode[] dnm = DayNightMode.values();
		entries = new String[dnm.length];
		for (int i = 0; i < entries.length; i++) {
			entries[i] = dnm[i].toHumanString(getMyApplication());
		}

		registerListPreference(settings.DAYNIGHT_MODE, screen, entries, dnm);

		String incompleteSuffix = " (" + getString(R.string.incomplete_locale)
				+ ")";
		// getResources().getAssets().getLocales();
		entrieValues = new String[] { "", "pt", "en", "fr", "it", "de", "es",
				"ru" };
		entries = new String[] { getString(R.string.system_locale),
				"Portuguese", "English", "French" + incompleteSuffix,
				"Italian" + incompleteSuffix, "German" + incompleteSuffix,
				"Spanish" + incompleteSuffix, "Russian" + incompleteSuffix, };
		/*
		 * entrieValues = new String[] { "", "en", "af", "hy", "eu", "be", "bs",
		 * "bg", "ca", "cs", "da", "nl", "fi", "fr", "ka", "de", "el", "iw",
		 * "hi", "hu", "id", "it", "ja", "ko", "lv", "lt", "mr", "no", "pl",
		 * "pt", "ro", "ru", "sk", "sl", "es", "sv", "tr", "uk", "vi", "cy" };
		 * entries = new String[] { getString(R.string.system_locale),
		 * "English", "Afrikaans", "Armenian" + incompleteSuffix, "Basque" +
		 * incompleteSuffix, "Belarusian" + incompleteSuffix, "Bosnian" +
		 * incompleteSuffix, "Bulgarian" + incompleteSuffix, "Catalan", "Czech",
		 * "Danish", "Dutch", "Finnish" + incompleteSuffix, "French",
		 * "Georgian", "German", "Greek", "Hebrew", "Hindi" + incompleteSuffix,
		 * "Hungarian", "Indonesian" + incompleteSuffix, "Italian", "Japanese" +
		 * incompleteSuffix, "Korean" + incompleteSuffix, "Latvian",
		 * "Lithuanian", "Marathi" + incompleteSuffix, "Norwegian" +
		 * incompleteSuffix, "Polish", "Portuguese", "Romanian", "Russian",
		 * "Slovak", "Slovenian", "Spanish", "Swedish", "Turkish", "Ukrainian",
		 * "Vietnamese", "Welsh" + incompleteSuffix };
		 */
		registerListPreference(settings.PREFERRED_LOCALE, screen, entries,
				entrieValues);

		ApplicationMode[] appModes = ApplicationMode.values(settings).toArray(
				new ApplicationMode[0]);
		entries = new String[appModes.length];
		for (int i = 0; i < entries.length; i++) {
			entries[i] = appModes[i].toHumanString(getMyApplication());
		}
		registerListPreference(settings.APPLICATION_MODE, screen, entries,
				appModes);
		...
		if (!Version.isBlackberry((ClientContext) getApplication())) {
			...
			if (nav == Configuration.NAVIGATION_DPAD
					|| nav == Configuration.NAVIGATION_TRACKBALL
					|| nav == Configuration.NAVIGATION_WHEEL
					|| nav == Configuration.NAVIGATION_UNDEFINED) {
				cat.addPreference(createCheckBoxPreference(
			...
			ListPreference lp = createListPreference(
					settings.AUDIO_STREAM_GUIDANCE, new String[] {
					...
					R.string.choose_audio_stream,
					R.string.choose_audio_stream_descr);
			final OnPreferenceChangeListener prev = lp
			...
			lp.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
				...
				public boolean onPreferenceChange(Preference preference,
					...
					if (player != null) {
						player.updateAudioStream(settings.AUDIO_STREAM_GUIDANCE
								.get());
					}
					return true;
				}
		...
		applicationModePreference = (ListPreference) screen
				.findPreference(settings.APPLICATION_MODE.getId());
		applicationModePreference.setOnPreferenceChangeListener(this);
		drivingRegionPreference = (ListPreference) screen
				.findPreference(settings.DRIVING_REGION.getId());
	}

	private void updateApplicationDirTextAndSummary() {
		if (applicationDir != null) {
			String storageDir = settings.getExternalStorageDirectory()
					.getAbsolutePath();
	...
	public boolean onPreferenceChange(Preference preference, Object newValue) {
		...
		if (id.equals(settings.SAFE_MODE.getId())) {
			...
			settings.DEFAULT_APPLICATION_MODE.set(settings.APPLICATION_MODE
					.get());
			updateAllSettings();
			...
			finish();
			startActivity(intent);
		}else if (id.equals(settings.DAYNIGHT_MODE.getId())) {
	...
	private void warnAboutChangingStorage(final String newValue) {
		...
		if (!path.canRead() || !path.exists()) {
			AccessibleToast.makeText(this, R.string.specified_dir_doesnt_exist,
					Toast.LENGTH_LONG).show();
			return;
		}
			...
			new AsyncTask<Void, Void, Void>() {
				...
				protected void onPostExecute(Void result) {
					...
					if (!NativeOsmandLibrary.isNativeSupported(storage,
							getMyApplication())) {
						AccessibleToast.makeText(SettingsGeneralActivity.this,
