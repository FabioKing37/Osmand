...
public class ShowRouteInfoActivity extends OsmandListActivity {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		if (item.getItemId() == SAVE) {
			MapActivityActions.createSaveDirections(ShowRouteInfoActivity.this)
					.show();
			return true;
		}
	...
	public void onListItemClick(ListView parent, View v, int position, long id) {
		...
		RouteDirectionInfo item = ((RouteInfoAdapter) getListAdapter())
				.getItem(position - 1);
		Location loc = helper.getLocationFromRouteDirection(item);
		if (loc != null) {
			RouteInfoLayer.directionInfo = position - 1;
			OsmandSettings settings = ((OsmandApplication) getApplication())
					.getSettings();
			settings.setMapLocationToShow(
					...
					null,
					item.getDescriptionRoute() + " " + getTimeDescription(item),
					null);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		createMenuItem(menu, SAVE, R.string.save_route_as_gpx,
				...
				R.drawable.ic_action_gsave_dark, MenuItem.SHOW_AS_ACTION_ALWAYS);
	...
	class RouteInfoAdapter extends ArrayAdapter<RouteDirectionInfo> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			TurnPathHelper.RouteDrawable drawable = new TurnPathHelper.RouteDrawable(
					getResources());
			...
			if (model.getDescriptionRoute().isEmpty())
				label.setText(R.string.map_widget_distance);
			else
