...
public class SearchActivity extends SherlockFragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		TabSpec addressSpec = tabHost.newTabSpec(SEARCH_ADDRESS).setIndicator(
				getTabIndicator(tabHost, R.drawable.tab_search_address_icon,
						R.string.address));
		mTabsAdapter.addTab(addressSpec,
				searchOnLine ? SearchAddressOnlineFragment.class
		...
		TabSpec favoriteTab = tabHost.newTabSpec(SEARCH_FAVORITES)
				.setIndicator(
						getTabIndicator(tabHost,
								R.drawable.tab_search_favorites_icon,
								R.string.favorite));
	...
	private void setTopSpinner() {
		spinnerAdapter = new ArrayAdapter<String>(getSupportActionBar()
					...
					public boolean onNavigationItemSelected(int position,
						...
						if (position != 0) {
							if (position == POSITION_CURRENT_LOCATION) {
								net.osmand.Location loc = getLocationProvider()
								...
								if (position == POSITION_LAST_MAP_VIEW) {
									...
									intent.putExtra(
											FavouritesListFragment.SELECT_FAVORITE_POINT_INTENT_KEY,
											(Serializable) null);
									...
									intent.setFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT);
									intent.putExtra(
											SearchAddressFragment.SELECT_ADDRESS_POINT_INTENT_KEY,
											(String) null);
									startActivityForResult(intent,
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_FAVORITE_SELECT
				&& resultCode == FavouritesListFragment.SELECT_FAVORITE_POINT_RESULT_OK) {
			FavouritePoint p = (FavouritePoint) data
					.getSerializableExtra(FavouritesListFragment.SELECT_FAVORITE_POINT_INTENT_KEY);
			if (p != null) {
			...
			String name = data
					.getStringExtra(SearchAddressFragment.SELECT_ADDRESS_POINT_INTENT_KEY);
			LatLon latLon = new LatLon(data.getDoubleExtra(
					SearchAddressFragment.SELECT_ADDRESS_POINT_LAT, 0),
					data.getDoubleExtra(
							SearchAddressFragment.SELECT_ADDRESS_POINT_LON, 0));
			if (name != null) {
	...
	public static class TabsAdapter extends FragmentPagerAdapter implements
		...
		public TabsAdapter(FragmentActivity activity, TabHost tabHost,
				TextView tabinfo, ViewPager pager) {
		...
		public void onTabChanged(String tabId) {
			...
			if (SEARCH_POI.equals(tabId)) {
				...
				tabInfo.setText(searchOnLine ? R.string.search_osm_nominatim
						: R.string.address_search_desc);
