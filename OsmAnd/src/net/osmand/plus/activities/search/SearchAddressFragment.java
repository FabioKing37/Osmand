...
public class SearchAddressFragment extends SherlockFragment {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		boolean light = ((OsmandApplication) getApplication()).getSettings()
				.isLightActionBar();
		if (getActivity() instanceof SearchAddressActivity) {
			...
			// WayPoints
			/*
			 * TargetPointsHelper targets = ((OsmandApplication)
			 * getApplication()).getTargetPointsHelper(); if
			 * (targets.getPointToNavigate() != null) { menuItem = menu.add(0,
			 * ADD_WAYPOINT, 0,
			 * R.string.context_menu_item_intermediate_point).setShowAsActionFlags
			 * ( MenuItem.SHOW_AS_ACTION_ALWAYS |
			 * MenuItem.SHOW_AS_ACTION_WITH_TEXT); menuItem =
			 * menuItem.setIcon(light ? R.drawable.ic_action_flage_dark :
			 * R.drawable.ic_action_flage_dark); } else { menuItem = menu.add(0,
			 * ADD_WAYPOINT, 0,
			 * R.string.context_menu_item_destination_point).setShowAsActionFlags
			 * ( MenuItem.SHOW_AS_ACTION_ALWAYS |
			 * MenuItem.SHOW_AS_ACTION_WITH_TEXT); menuItem =
			 * menuItem.setIcon(light ? R.drawable.ic_action_flag_dark :
			 * R.drawable.ic_action_flag_dark); }
			 * menuItem.setOnMenuItemClickListener(new OnMenuItemClickListener()
			 * {
			 * 
			 * @Override public boolean
			 * onMenuItemClick(com.actionbarsherlock.view.MenuItem item) {
			 * select(ADD_WAYPOINT); return true; } });
			 */
			menuItem = menu
			...
			// ONLINE_SEARCH
			/*
			 * menuItem = menu.add(0, ONLINE_SEARCH, 0,
			 * R.string.search_online_address
			 * ).setShowAsActionFlags(MenuItem.SHOW_AS_ACTION_ALWAYS |
			 * MenuItem.SHOW_AS_ACTION_WITH_TEXT); menuItem =
			 * menuItem.setIcon(light ? R.drawable.ic_action_gnext_dark :
			 * R.drawable.ic_action_gnext_dark);
			 * menuItem.setOnMenuItemClickListener(new OnMenuItemClickListener()
			 * {
			 * 
			 * @Override public boolean
	...
	public static class AddressInformation {
		...
		public static AddressInformation build2StreetIntersection(Context ctx,
			...
			ai.historyName = MessageFormat.format(
					ctx.getString(R.string.search_history_int_streets),
					settings.getLastSearchedStreet(),
					settings.getLastSearchedIntersectedStreet(), cityName);
			ai.zoom = 17;
			return ai;
		}

		public static AddressInformation buildStreet(Context ctx,
				OsmandSettings settings) {
			AddressInformation ai = new AddressInformation();
			String postcode = settings.getLastSearchedPostcode();
			String city = settings.getLastSearchedCityName();
			...
			ai.historyName = MessageFormat.format(
					ctx.getString(R.string.search_history_street), street,
					cityName);
			ai.zoom = 16;
			return ai;
		}

		public static AddressInformation buildBuilding(Context ctx,
				OsmandSettings settings) {
			AddressInformation ai = new AddressInformation();
			String postcode = settings.getLastSearchedPostcode();
			String city = settings.getLastSearchedCityName();
			...
			ai.historyName = MessageFormat.format(
					ctx.getString(R.string.search_history_building), building,
					street, cityName);
			ai.zoom = 17;
			return ai;
		}

		public static AddressInformation buildCity(Context ctx,
				OsmandSettings settings) {
			AddressInformation ai = new AddressInformation();
			String city = settings.getLastSearchedCityName();
			ai.historyName = MessageFormat.format(
					ctx.getString(R.string.search_history_city), city);
			ai.objectName = city;
			ai.zoom = 14;
			return ai;
		}
	...
	public void select(int mode) {
		if (searchPoint == null) {
			AccessibleToast.makeText(getActivity(),
					R.string.please_select_address, Toast.LENGTH_SHORT).show();
			return;
		}
		AddressInformation ai = new AddressInformation();
		if (!Algorithms.isEmpty(street2) && !Algorithms.isEmpty(street)) {
			ai = AddressInformation.build2StreetIntersection(getActivity(),
					osmandSettings);
		} else if (!Algorithms.isEmpty(building)) {
			ai = AddressInformation
					.buildBuilding(getActivity(), osmandSettings);
		...
		if (mode == ADD_TO_FAVORITE) {
			Bundle b = new Bundle();
			Dialog dlg = MapActivityActions.createAddFavouriteDialog(
					getActivity(), b);
			dlg.show();
			MapActivityActions.prepareAddFavouriteDialog(getActivity(), dlg, b,
			...
			intent.putExtra(SELECT_ADDRESS_POINT_LON,
	...
	protected void updateBuildingSection() {
		if (radioBuilding) {
			...
			if (Algorithms.isEmpty(building)) {
				// ((TextView)
				// findViewById(R.id.BuildingButton)).setTextColor(getResources().getColor(R.color.mapngo_color));
			...
			// ((TextView)
			// findViewById(R.id.BuildingText)).setTextColor(getResources().getColor(R.color.mapngo_color));
			((TextView) findViewById(R.id.BuildingText))
		...
		findViewById(R.id.ResetBuilding).setEnabled(
				!Algorithms.isEmpty(street2) || !Algorithms.isEmpty(building));
	}
	...
	protected void updateUI() {
		...
		if (Algorithms.isEmpty(region)) {
			countryButton.setTextColor(getResources().getColor(
