...
public class SearchCityByNameActivity extends
		SearchByNameAbstractActivity<City> {
	...
	protected void addFooterViews() {
		...
		searchVillages.setTextColor(getResources().getColor(
				R.color.color_white));
	...
	protected void filterLoop(String query, Collection<City> list) {
		...
		if (!initializeTaskIsFinished()
				|| (query.length() <= 3 && !searchVillages())) {
	...
	public String getText(City obj) {
		...
		if (getFilter().length() > 2 && locationToSearch != null && l != null) {
			...
			if (obj.getType() != null) {
				name += " ["
						+ OsmAndFormatter.toPublicString(obj.getType(),
								getMyApplication()) + "]";
			}
			return name
					+ " - " + //$NON-NLS-1$
					OsmAndFormatter.getFormattedDistance(
							(int) MapUtils.getDistance(l, locationToSearch),
							getMyApplication());
		} else {
	...
	public void itemSelected(City obj) {
		settings.setLastSearchedCity(obj.getId(),
				obj.getName(region.useEnglishNames()), obj.getLocation());
		if (region.getCityById(obj.getId(),
				obj.getName(region.useEnglishNames())) == null) {
	...
	private final class CityComparator implements Comparator<City> {
		...
		private CityComparator(StringMatcherMode startsWith, boolean en) {
		...
		public int compare(City lhs, City rhs) {
			...
			boolean st1 = CollatorStringMatcher.cmatches(cs, lhs.getName(en),
					part, startsWith);
			boolean st2 = CollatorStringMatcher.cmatches(cs, rhs.getName(en),
					part, startsWith);
			if (st1 != st2) {
				return st1 ? 1 : -1;
			}
			...
			if (locationToSearch != null) {
				double d1 = MapUtils.getDistance(locationToSearch,
						lhs.getLocation());
				double d2 = MapUtils.getDistance(locationToSearch,
						rhs.getLocation());
		...
		private int compareCityType(City lhs, City rhs) {
			boolean c1 = lhs.getType() == CityType.CITY
					|| lhs.getType() == CityType.TOWN;
			boolean c2 = rhs.getType() == CityType.CITY
					|| rhs.getType() == CityType.TOWN;
			if (c1 == c2) {
				return 0;
			}
			return c1 ? 1 : -1;
