...
public class SearchPOIActivity extends OsmandListActivity implements
	...
	public static final String POI_NAME = "net.osmand.poi_name";
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		searchPOILevel = menu.add(0, SEARCH_MORE, 0,
				R.string.search_POI_level_btn).setShowAsActionFlags(
		...
		showFilterItem = showFilterItem
				.setIcon(light ? R.drawable.ic_action_filter_dark
				...
				.setOnMenuItemClickListener(new OnMenuItemClickListener() {
					...
					public boolean onMenuItemClick(
						...
						if (isSearchByNameFilter()) {
							...
							if (location != null) {
								newIntent.putExtra(
										EditPOIFilterActivity.SEARCH_LAT,
										location.getLatitude());
								newIntent.putExtra(
										EditPOIFilterActivity.SEARCH_LON,
		...
		showOnMapItem = menu.add(0, SHOW_ON_MAP, 0,
				R.string.search_shown_on_map).setShowAsActionFlags(
				MenuItem.SHOW_AS_ACTION_ALWAYS
						| MenuItem.SHOW_AS_ACTION_WITH_TEXT);
		showOnMapItem = showOnMapItem
				.setIcon(light ? R.drawable.ic_action_map_marker_dark
	...
	private boolean searchMore() {
		...
		if (query.length() < 2
			...
			AccessibleToast.makeText(SearchPOIActivity.this,
					R.string.poi_namefinder_query_empty, Toast.LENGTH_LONG)
					.show();
			return true;
		}
		if (isNameFinderFilter()
				&& !Algorithms.objectEquals(
						((NameFinderPoiFilter) filter).getQuery(), query)) {
			filter.clearPreviousZoom();
			((NameFinderPoiFilter) filter).setQuery(query);
			runNewSearchQuery(SearchAmenityRequest.buildRequest(location,
					SearchAmenityRequest.NEW_SEARCH_INIT));
		} else if (isSearchByNameFilter()
				&& !Algorithms.objectEquals(
						((SearchByNameFilter) filter).getQuery(), query)) {
			showFilterItem.setVisible(false);
			filter.clearPreviousZoom();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		boolean closePoi = intent.getBooleanExtra(POI_CLOSE, false);
		if (closePoi) {
	...
	public void onCreate(Bundle icicle) {
		...
		getSupportActionBar().setIcon(R.drawable.tab_search_location_icon);
	...
	private void updateSubtitle() {
		if (filter != null) {
			getSupportActionBar().setSubtitle(
					filter.getName() + " " + filter.getSearchArea());
		}
	}

	private void showPoiCategoriesByNameFilter(String query,
			net.osmand.Location loc) {
	...
	private String typesToString(Map<AmenityType, List<String>> map) {
		...
		while (iterator.hasNext() && count < 4) {
			...
			b.append("\n")
					.append(OsmAndFormatter.toPublicString(e.getKey(),
							getMyApplication())).append(" - ");
			if (e.getValue() == null) {
				b.append("...");
			} else {
	...
	private void updateSearchPoiTextButton(boolean taskAlreadyFinished) {
		...
		if (location == null) {
			...
			enabled = false;
		} else if (filter != null && !isNameFinderFilter()
				&& !isSearchByNameFilter()) {
		...
		if (ResourcesCompat.getResources_getBoolean(this,
	...
	public void onListItemClick(ListView parent, View v, int position, long id) {
		...
		String poiSimpleFormat = OsmAndFormatter.getPoiSimpleFormat(amenity,
				getMyApplication(), settings.usingEnglishNames());
		String name = poiSimpleFormat;
		int z = Math.max(16, settings.getLastKnownMapZoom());
		MapActivityActions.createDirectionsActions(qa, amenity.getLocation(),
				amenity, name, z, this, true, null);
		ActionItem poiDescription = new ActionItem();
		poiDescription.setIcon(getResources().getDrawable(
				R.drawable.ic_action_note_blue));
		poiDescription
				.setTitle(getString(R.string.poi_context_menu_showdescription));
		final String d = OsmAndFormatter.getAmenityDescriptionContent(
				getMyApplication(), amenity, false);
		poiDescription.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bs.setTitle(OsmAndFormatter.getPoiSimpleFormat(amenity,
						getMyApplication(), settings.USE_ENGLISH_NAMES.get()));
		...
		if (((OsmandApplication) getApplication()).getInternalAPI()
	...
	static class SearchAmenityRequest {
		...
		public static SearchAmenityRequest buildRequest(net.osmand.Location l,
				int type) {
	...
	class SearchAmenityTask extends AsyncTask<Void, Amenity, List<Amenity>>
		...
		protected void onPostExecute(List<Amenity> result) {
			...
			if (isNameFinderFilter()) {
				if (!Algorithms.isEmpty(((NameFinderPoiFilter) filter)
						.getLastError())) {
					AccessibleToast.makeText(SearchPOIActivity.this,
		...
		protected List<Amenity> doInBackground(Void... params) {
			if (request.location != null) {
				if (request.type == SearchAmenityRequest.NEW_SEARCH_INIT) {
					return filter.initializeNewSearch(
							request.location.getLatitude(),
							request.location.getLongitude(), -1, this);
				} else if (request.type == SearchAmenityRequest.SEARCH_FURTHER) {
	...
	class AmenityAdapter extends ArrayAdapter<Amenity> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			ImageView direction = (ImageView) row
					.findViewById(R.id.poi_direction);
			...
			if (loc != null) {
				...
				net.osmand.Location.distanceBetween(l.getLatitude(),
						l.getLongitude(), loc.getLatitude(),
						loc.getLongitude(), mes);
			}
			int opened = -1;
			if (amenity.getOpeningHours() != null) {
				OpeningHours rs = OpeningHoursParser.parseOpenedHours(amenity
						.getOpeningHours());
				if (rs != null) {
			...
			MapRenderingTypes.getDefault().getAmenityTagValue(
					amenity.getType(), amenity.getSubType(), tag, value);
			if (RenderingIcons.containsBigIcon(tag + "_" + value)) {
				icon.setImageResource(RenderingIcons.getBigIconResourceId(tag
						+ "_" + value));
			} else if (RenderingIcons.containsBigIcon(value.toString())) {
				icon.setImageResource(RenderingIcons.getBigIconResourceId(value
						.toString()));
			} else {
	...
	private void showPOIDetails(final Amenity amenity, boolean en) {
		...
		b.setTitle(OsmAndFormatter.getPoiSimpleFormat(amenity,
				getMyApplication(), en));
		b.setPositiveButton(R.string.default_buttons_ok,
		...
		NavigationInfo navigationInfo = app.getLocationProvider()
				.getNavigationInfo();
		String direction = navigationInfo.getDirectionString(
				amenity.getLocation(), heading);
		if (direction != null)
			attributes.add(direction);
		String[] as = OsmAndFormatter.getAmenityDescriptionContent(
				getMyApplication(), amenity, false).split("\n");
		for (String s : as) {
	...
	class DirectionDrawable extends Drawable {
		...
		public DirectionDrawable() {
			...
			paintRouteDirection.setColor(getResources().getColor(
					R.color.mapngo_color));
