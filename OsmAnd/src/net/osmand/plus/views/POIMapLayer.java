...
public class POIMapLayer extends OsmandMapLayer implements
	...
	public static final org.apache.commons.logging.Log log = PlatformUtil
	...
	public void setShopFilter(PoiFilter filter) {
		this.shopFilter = filter;
	}

	public PoiFilter getShopFilter() {
		return shopFilter;
	}

	public void getAmenityFromPoint(RotatedTileBox tb, PointF point,
		...
		if (objects != null) {
			...
			try {
				for (int i = 0; i < objects.size(); i++) {
					...
					int x = (int) tb.getPixXFromLatLon(n.getLocation()
							.getLatitude(), n.getLocation().getLongitude());
					int y = (int) tb.getPixYFromLatLon(n.getLocation()
							.getLatitude(), n.getLocation().getLongitude());
	...
	public boolean onSingleTap(PointF point, RotatedTileBox tileBox) {
		...
		if (!am.isEmpty()) {
			...
			AccessibleToast.makeText(view.getContext(), res.toString(),
	...
	private StringBuilder buildPoiInformation(StringBuilder res, Amenity n) {
		...
		res.append(" "
				+ format
				+ "\n"
				+ OsmAndFormatter.getAmenityDescriptionContent(
						view.getApplication(), n, true));
		return res;
	}
	...
	public void initLayer(OsmandMapTileView view) {
		...
		pointAltUI.setColor(view.getApplication().getResources()
				.getColor(R.color.poi_background));
		...
		point.setColor(view.getApplication().getResources()
				.getColor(R.color.mapngo_color));
	...
	public void onPrepareBufferImage(Canvas canvas, RotatedTileBox tileBox,
		...
		if (tileBox.getZoom() >= startZoom) {
			...
			resourceManager.searchAmenitiesAsync(latLonBounds.top,
					...
					tileBox.getZoom(), filter, objects);
			int r = getRadiusPoi(tileBox);
			for (Amenity o : objects) {
				int x = (int) tileBox.getPixXFromLatLon(o.getLocation()
						.getLatitude(), o.getLocation().getLongitude());
				int y = (int) tileBox.getPixYFromLatLon(o.getLocation()
						.getLatitude(), o.getLocation().getLongitude());
				...
				MapRenderingTypes.getDefault().getAmenityTagValue(o.getType(),
						o.getSubType(), tag, value);
			...
			if (view.getSettings().SHOW_POI_LABEL.get()) {
				...
				for (Amenity o : objects) {
					...
					int y = (int) tileBox.getPixYFromLatLon(o.getLocation()
							.getLatitude(), o.getLocation().getLongitude());
					int tx = tileBox.getPixXFromLonNoRot(o.getLocation()
							.getLongitude());
					int ty = tileBox.getPixYFromLatNoRot(o.getLocation()
							.getLatitude());
					String name = o
							.getName(view.getSettings().USE_ENGLISH_NAMES.get());
	...
	public void populateObjectContextMenu(Object o, ContextMenuAdapter adapter) {
		if (o instanceof Amenity) {
				...
				public void onContextMenuClick(int itemId, int pos,
					...
					if (itemId == R.string.poi_context_menu_call) {
						try {
							...
							AccessibleToast.makeText(view.getContext(),
			...
			if (OsmAndFormatter.getAmenityDescriptionContent(
					view.getApplication(), a, false).length() > 0) {
				adapter.item(R.string.poi_context_menu_showdescription)
						.icons(R.drawable.ic_action_note_dark,
								R.drawable.ic_action_note_light)
						.listen(listener).reg();
			}
			if (a.getPhone() != null) {
				adapter.item(R.string.poi_context_menu_call)
						.icons(R.drawable.ic_action_call_dark,
								R.drawable.ic_action_call_light)
						.listen(listener).reg();
			}
			if (a.getSite() != null) {
				adapter.item(R.string.poi_context_menu_website)
						.icons(R.drawable.ic_action_globus_dark,
								R.drawable.ic_action_globus_light)
						.listen(listener).reg();
			}
		}
	}
	...
	private void showDescriptionDialog(Amenity a) {
		...
		bs.setTitle(OsmAndFormatter.getPoiSimpleFormat(a,
				view.getApplication(),
				view.getSettings().USE_ENGLISH_NAMES.get()));
		if (a.getType() == AmenityType.OSMWIKI) {
			bs.setMessage(a.getDescription());
		} else {
			bs.setMessage(OsmAndFormatter.getAmenityDescriptionContent(
					view.getApplication(), a, false));
		}
		bs.show();
	}
